! function(t) {
    ...
    const actionUrl = t.getAttribute('action');

    fetch(actionUrl, {
        method: 'POST',
        body: formData
    }).then(() => {
        // Перенаправляем через 500 мс (чтобы данные успели отправиться)
        setTimeout(() => {
            window.location.href = "https://trueremonter.tilda.ws/thanks ";
        }, 500);
    }).catch(err => {
        console.error('[TKFORM] Ошибка отправки:', err);
    });
});
